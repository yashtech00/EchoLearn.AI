...
export default function GoalsForm({ formData, setFormData, onNext }: GoalsFormProps) {
  ...
  const goals = [
    { id: "CASUAL", name: "Fluency", Icon: MessageCircle, description: "Speak naturally and confidently in any daily situation", featured: true },
    { id: "EXAM", name: "Exam Prep", Icon: ClipboardCheck, description: "IELTS, TOEFL, or Cambridge exams" },
    { id: "JOB_COMMUNICATION", name: "Business English", Icon: Building2, description: "Emails, meetings, and presentations" },
    { id: "RELOCATION", name: "Travel & Culture", Icon: Plane, description: "Explore the world with confidence" },
    { id: "ACADEMIC", name: "Grammar Mastery", Icon: Brain, description: "Master English grammar rules" },
  ];
              ...
              >
                <div className="relative z-10">
                  <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-7 transition-all duration-300 ${
                    formData.primaryGoal === "CASUAL" ? 'bg-white/20 text-white backdrop-blur-sm' : 'bg-indigo-100 text-indigo-600 group-hover:scale-110'
                  }`}>
                  ...
                  <p className={`text-lg leading-relaxed mb-7 ${
                    formData.primaryGoal === "CASUAL" ? 'text-white/95' : 'text-gray-600'
                  }`}>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest ${
                      formData.primaryGoal === "CASUAL" ? 'bg-white/25 text-white backdrop-blur-sm' : 'bg-indigo-100 text-indigo-700'
                    }`}>
                      Most Popular
                    </span>
                  </div>
                ...
                <div className={`absolute -right-16 -bottom-16 w-72 h-72 rounded-full blur-3xl transition-all duration-500 ${
                  formData.primaryGoal === "CASUAL" ? 'bg-white/15' : 'bg-indigo-100/30 group-hover:bg-indigo-200/40'
                }`}></div>
              </div>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            {formData.primaryGoal === goal.id && (
                              <CheckCircle2 className="w-5 h-5 text-indigo-600 fill-indigo-600 animate-in zoom-in duration-300" />
                            )}
                          </div>
