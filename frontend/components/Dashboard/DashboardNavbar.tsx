...
import { Search, Flame, Zap, Menu } from "lucide-react";
...
export default function DashboardNavbar({ onToggleSidebar }: DashboardNavbarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 md:left-64 right-0 h-16 bg-background border-b border-border z-40 font-sans transition-all duration-300">
      <div className="h-full px-4 md:px-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 md:gap-3 overflow-x-auto scrollbar-hide flex-1">
          {onToggleSidebar && (
            <button
              ...
              className="md:hidden p-2 rounded-lg hover:bg-primary/5 text-foreground/70 hover:text-primary transition-all flex-shrink-0"
          ...
          <div className="flex items-center gap-1.5 md:gap-2 overflow-x-auto scrollbar-hide py-1">
            {navbarItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 sm:px-4 py-2 rounded-[12px] text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-200 ${
                    isActive 
        ...
        <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
          {/* SEARCH */}
          <div className="hidden lg:flex items-center gap-2 bg-primary/5 border border-border rounded-[12px] px-4 py-2 w-64 xl:w-80">
          ...
          {/* STREAK */}
          <div className="flex items-center gap-1.5 sm:gap-2 bg-orange-500/10 text-orange-600 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-[12px] border border-orange-500/20">
            <Flame className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${(stats?.currentStreak || 0) > 0 ? "fill-orange-500" : ""}`} />
